      ...
      {products.map((product) => {
        return (
          <div key={product.id} className="flex-col justify-center shadow-sm">
            <Image
              className="w-full object-cover bg-gradient-to-r from-indigo-500 to-green-400 rounded-t-md h-[300px]"
            ...
            <footer className="grid justify-center items-center bg-gray-800 text-center font-sans rounded-md]">
              ...
              <span className="block text-[1.5rem] text-green-400">
                {product.price}
              </span>
              <Link
                href={`/product/${product.id}`}
                className="bg-blue-900 flex justify-center items-center gap-[0.3rem] w-[16rem] h-[3.4rem] m-auto mb-[1rem]  rounded-md font-bold text-[1.5rem] text-center hover:bg-blue-950 transition-colors duration-300 "
              >
                COMPRAR
